...
public class EcommerceApplication implements CommandLineRunner {
		...
		while (loggedInUser != null) {
			...
			try {
				switch (choice) {
					...
					default -> log.info("Invalid Choice for Admin Dashboard");
				}
	...
	private void addNewProduct(Scanner scanner) {
		...
		for (Category category : categories) {
			log.info("{}. {}", category.getCategoryId(), category.getCategoryName());
		}
		...
		while (loggedInUser != null) {
			...
			try {
				switch (choice) {
					...
					default -> log.info("Invalid Choice for User Dashboard");
				}
			} catch (RuntimeException ex) {
				log.info(ex.getMessage());
			}
	...
	private void searchProducts(Scanner scanner) {
		...
		products.forEach(product ->
				log.info("ID: {} | Name: {} | Description: {} | Price: {} | Category: {}",
						...
						product.getPrice(),
						categoryService.getCategoryById(product.getCategoryId()).getCategoryName()));
	}
